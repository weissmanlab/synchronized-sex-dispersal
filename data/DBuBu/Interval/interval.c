...
typedef struct
{
    ...
    unsigned int numloci, tgap, tint;
...
int main(int argc, char *argv[])
{
    ...
    unsigned int L, d, parity, Tgap, **mutcount, run, fittgap = 1, **max_fit_ind, **max_fit_deme, **cap_size, dist, relateness;
    ...
    if (argc != 11)
    {
        printf("Usage: s tgap U V D N L tfinal tint ranseed outfile\n");
        return EXIT_FAILURE;
    }
    j = 1;
    ...
    params->tint = (unsigned long)atoll(argv[j++]);
    ...
    for (; params->tint <= Tgap; params->tint += Tgap)
    {
        Tgap = params->tgap;
        params->frac = 0.0025 * Tgap;
        //Print out variables to paramfile:
        sprintf(outfile, "%u%c%u%c%ld%c%ld%c%s%c%s", params->tgap, 'I', params->tint, 'D', params->numdemes, 'N', params->popsize, 'M', mutn, 'R', recn);
        strcpy(params->name, outfile);
        ...
        fprintf(paramfile, "ranseed = %ld,frac = %lf,tgap = %u,tint = %u,s = %lf,U = %lf,V = %lf,D = %lu,N = %lu,L = %u,tfinal = %lu\n", SEED, params->frac, params->tgap, params->tint, params->s, params->mutrate, params->recrate, params->numdemes, N, L, tfinal);
...
int generation(parameters *params, char **population, double **W, unsigned int **mutcount, double *sels, unsigned long t, gsl_rng *RNDM)
{
    ...
    int vargen = 0, migrants = 0, recombine = 0;
    double rec = 0;
    ...
    if ((t - params->tint + params->tgap) % params->tgap == params->tgap - 1)
    {
